...
export default function News({ onOpenPress }) {
  ...
  const renderTitle = (item) => {
    if (item.press) {
      return (
        <button onClick={() => onOpenPress?.(item.press)}
          style={{ textAlign: 'left', background: 'transparent', border: 'none', padding: 0, cursor: 'pointer', fontSize: '0.92rem', color: 'var(--text)', fontWeight: 400, borderBottom: '1px dashed rgba(0,160,232,0.4)', fontFamily: 'inherit', transition: 'color 0.2s, border-color 0.2s' }}
    ...
    return <span style={{ fontSize: '0.92rem', color: 'var(--text)', fontWeight: 400 }}>{item.title}</span>
  }
  ...
  const renderAction = (item) => {
    if (item.press) {
      return (
        <button onClick={() => onOpenPress?.(item.press)}
          style={{ background: 'transparent', border: 'none', padding: 0, cursor: 'pointer', fontFamily: 'Inter, sans-serif', fontSize: '0.78rem', fontWeight: 600, color: 'var(--accent)', whiteSpace: 'nowrap' }}>
          詳細 →
        </button>
      )
    }
    if (item.url) {
      return <a href={item.url} target="_blank" rel="noopener" style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.78rem', fontWeight: 600, color: 'var(--accent)', whiteSpace: 'nowrap' }}>詳細 →</a>
    }
    return <span />
  }
